...
import { PropertyNameOptions, Mapper } from './interfaces';
import { StubMap } from './stub-map';
import { DateMapper } from './date-mapper';

@Injectable()
export class Js2Json {

  private static readonly stubMap: Map<any, any> = new StubMap();

  private readonly logger = new Logger(Js2Json.name);
  ...
  /**
   * This component allow converting Nodejs (+Typescript) objects to
   * raw objects for HTTP interface o DB repository, ie the reverse
   * of [Json2Js]{@link ./json2js.ts#Json2js}
   * Use [Json2JsProperty]{@link ./decorators.ts#Json2JsProperty} and
   * [Json2JsStrategy]{@link ./decorators.ts#Json2JsStrategy} to add
   * corresponding metadata
   * Js2Json is able to detect 'Maximum call stack size exceeded'
   ...
   * due JSON does not allow circular or recursive references it is
   * posible that JSON.stringify fail on `TypeError: Converting circular
   * structure to JSON`
   */
  map<T>(
    source: any,
    ...
    const cache = graph ? new Map() : Js2Json.stubMap;
    try {
  ...
  private mapInternal<T>(
    ...
    if (Array.isArray(type)) {
      if (Array.isArray(source)) {
        return source.map((x) => this.mapInternal(x, type[0], cache, property));
      }
      return null;
    }
    ...
    if (type.name === 'Date') {
      if (source instanceof Date) {
        return <T><unknown>this.dateMapper.toJson(source, property);
      }
      return null;
    }
  ...
  private attribute(targetKey: string, property: PropertyNameOptions) {
    return (source: {}, target: {}, cache: Map<any, any>) => {
      if (source.hasOwnProperty(property.key)) {
        const value = source[property.key];
        target[targetKey] = this.mapInternal(value, property.type, cache, property);
      }
    };
  }
